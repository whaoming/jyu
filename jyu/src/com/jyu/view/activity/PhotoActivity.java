...
public class PhotoActivity extends Activity {
		...
		if (!isNet) {
			...
			photo_bt_xuan.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					/* 创建新的Bitmap对象 */
					Bitmap resizedBitmap = Bitmap.createBitmap(mySourceBmp, 0,
							0, widthOrig, heightOrig, matrix, true);
					...
					((ImageView) listViews.get(count))
							.setImageDrawable(myNewBitmapDrawable);
			...
			photo_bt_xuan.setVisibility(View.GONE);
	...
	@SuppressWarnings("deprecation")
	private void intListViewByNet(String url, int i) {
		if (listViews == null)
			listViews = new ArrayList<View>();
		img = new ImageView(this);// 构造textView对象
		img.setBackgroundColor(0xff000000);
		bitmapUtil = new BitmapUtils(PhotoActivity.this);
		if(!url.startsWith("/image")){
			bitmapUtil.display(img, url);
		}else{
			bitmapUtil.display(img, ConstantValue.LOTTERY_URI+url);
		}
